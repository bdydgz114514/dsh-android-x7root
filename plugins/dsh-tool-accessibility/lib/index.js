...
function apply(ctx) {
  // v1.7.5-x7root.3：把「先读屏定位、再触控」的工作流写进系统提示
  ctx.inject(["systemPrompt"], (pctx) => {
    pctx.systemPrompt.section({
      name: "tool-accessibility:instructions",
      order: 940,
      text: [
        "## 屏幕操作（无障碍读屏 + 触控）",
        "需要看/操作手机界面时：先 android_a11y_status 确认服务已开启；再用 android_find（按文字/描述/类名精确定位，直接给出中心坐标）" +
        "或 android_screen（整屏控件树）确定目标；然后 android_tap 点击（优先 fx/fy 分数坐标，免疫截图缩放）、" +
        "android_type 输入（WebView/网页输入框用 paste:true）、android_scroll / android_swipe 滚动滑动。",
        "系统级手势用 android_gesture_preset（back/home/recents/notifications/quick_settings/scroll_top/scroll_bottom/zoom_in/zoom_out/double_tap/long_press）；" +
        "多指或游戏操作用 android_gesture（多笔时间轴）与 android_touch；看画面用 android_see（可叠加网格便于定位）。"
      ].join("\n")
    });
  });
  ...
  // v1.7.5-x7root.3：UI 元素定位（按文字/描述/类名查控件，返回中心坐标，不点击）
  ctx.tools.register(defineTool({
    name: "android_find",
    description:
      "在当前屏幕控件树里定位元素：按 text（文字）/desc（内容描述）/cls（类名，如 EditText）模糊包含匹配，" +
      "返回匹配节点的标签、中心坐标 center=(x,y)、bounds 与可点击/可输入标志（只查找、不点击）。" +
      "拿到中心坐标后再用 android_tap 点击（推荐换算成 fx/fy 分数坐标）。clickableOnly=true 只看可点击项。",
    parameters: {
      text: { type: "string", description: "控件文字（模糊包含匹配）" },
      desc: { type: "string", description: "控件内容描述（模糊包含匹配）" },
      cls: { type: "string", description: "控件类名（模糊包含匹配，如 EditText/Button）" },
      clickableOnly: { type: "boolean", description: "只返回可点击节点（默认 false）" },
    ...
    output: {
      ...
      render: (_a, v) => [{ type: "text", text: v.ok ? v.text : renderResult(v)[0].text }]
    },
    async execute(args) {
      const raw = await a11yRequest("/dump", undefined, 8000);
      const v = parseResult(raw);
      ...
      const inc = (x, q) => typeof x === "string" && x.toLowerCase().indexOf(q.toLowerCase()) >= 0;
      const qText = typeof args.text === "string" ? args.text.trim() : "";
      const qDesc = typeof args.desc === "string" ? args.desc.trim() : "";
      const qCls = typeof args.cls === "string" ? args.cls.trim() : "";
      const limit = typeof args.limit === "number" && args.limit > 0 ? Math.min(args.limit, 100) : 20;
      const matches = nodes.filter((n) => {
        if (args.clickableOnly === true && n.clickable !== true) return false;
        const parts = [];
        if (qText !== "") parts.push(inc(n.text, qText));
        if (qDesc !== "") parts.push(inc(n.desc, qDesc));
        if (qCls !== "") parts.push(inc(n.cls, qCls));
        if (parts.length === 0) return false;
        for (const p of parts) if (!p) return false;
        return true;
      }).slice(0, limit);
      if (matches.length === 0) {
        return { ok: true, package: v.package || "", count: 0, text: "未找到匹配的控件；可用 android_screen 查看完整控件树，或用 android_see 看画面。" };
      }
      const lines = matches.map((n, i) => {
        const label = n.text || n.desc || "(无文字)";
        const cx = Math.round(Number(n.x || 0) + Number(n.w || 0) / 2);
        const cy = Math.round(Number(n.y || 0) + Number(n.h || 0) / 2);
        const flags = [];
        if (n.clickable) flags.push("可点击");
        if (n.input) flags.push("可输入");
        if (n.scrollable) flags.push("可滚动");
        return "[" + i + "] " + label + "  center=(" + cx + "," + cy + ")  bounds=(" + Number(n.x || 0) + "," + Number(n.y || 0) + "," + Number(n.w || 0) + "x" + Number(n.h || 0) + ")" + (flags.length ? " " + flags.join("/") : "");
      });
      return { ok: true, package: v.package || "", count: matches.length, text: lines.join("\n") };
    }
  }));
  ...
  // v1.7.5-x7root.3：预置手势（一键完成常用系统手势/多指操作）
  const PRESET_GESTURES = {
    back: () => [{ kind: "swipe", finger: 0, fx: 0.02, fy: 0.5, fx2: 0.4, fy2: 0.5, durationMs: 250 }],
    home: () => [{ kind: "swipe", finger: 0, fx: 0.5, fy: 0.995, fx2: 0.5, fy2: 0.85, durationMs: 250 }],
    recents: () => [
      { kind: "down", finger: 0, fx: 0.5, fy: 0.995 },
      { kind: "move", finger: 0, fx: 0.5, fy: 0.6, durationMs: 350 },
      { kind: "wait", ms: 400 },
      { kind: "up", finger: 0 }
    ],
    notifications: () => [{ kind: "swipe", finger: 0, fx: 0.5, fy: 0.005, fx2: 0.5, fy2: 0.7, durationMs: 350 }],
    quick_settings: () => [{ kind: "swipe", finger: 0, fx: 0.85, fy: 0.005, fx2: 0.85, fy2: 0.85, durationMs: 350 }],
    scroll_top: () => [{ kind: "swipe", finger: 0, fx: 0.5, fy: 0.25, fx2: 0.5, fy2: 0.9, durationMs: 300 }],
    scroll_bottom: () => [{ kind: "swipe", finger: 0, fx: 0.5, fy: 0.85, fx2: 0.5, fy2: 0.2, durationMs: 300 }],
    zoom_in: () => [
      { kind: "down", finger: 0, fx: 0.48, fy: 0.5 },
      { kind: "down", finger: 1, fx: 0.52, fy: 0.5 },
      { kind: "move", finger: 0, fx: 0.3, fy: 0.5, durationMs: 300 },
      { kind: "move", finger: 1, fx: 0.7, fy: 0.5, durationMs: 300 },
      { kind: "up", finger: 0 },
      { kind: "up", finger: 1 }
    ],
    zoom_out: () => [
      { kind: "down", finger: 0, fx: 0.3, fy: 0.5 },
      { kind: "down", finger: 1, fx: 0.7, fy: 0.5 },
      { kind: "move", finger: 0, fx: 0.48, fy: 0.5, durationMs: 300 },
      { kind: "move", finger: 1, fx: 0.52, fy: 0.5, durationMs: 300 },
      { kind: "up", finger: 0 },
      { kind: "up", finger: 1 }
    ],
    pinch_in: () => [
    ...
    double_tap: (fx, fy) => [
      { kind: "tap", finger: 0, fx: fx, fy: fy, durationMs: 60 },
      { kind: "wait", ms: 90 },
      { kind: "tap", finger: 0, fx: fx, fy: fy, durationMs: 60 }
    ],
    long_press: (fx, fy, d) => [{ kind: "hold", finger: 0, fx: fx, fy: fy, durationMs: d }]
  };

  ctx.tools.register(defineTool({
    name: "android_gesture_preset",
    description:
      "执行预置手势（无需自己拼坐标）：back=返回、home=回桌面、recents=最近任务、notifications=下拉通知栏、" +
      "quick_settings=下拉快捷设置、scroll_top=滚到顶部、scroll_bottom=滚到底部、zoom_in/pinch_out=双指放大、" +
      "zoom_out/pinch_in=双指缩小、double_tap=双击、long_press=长按。可选 fx/fy（0~1 锚点，double_tap/long_press 用）" +
      "与 durationMs（长按/滑动时长覆盖）。需要已开启无障碍服务。",
    parameters: {
      preset: { type: "string", required: true, enum: ["back", "home", "recents", "notifications", "quick_settings", "scroll_top", "scroll_bottom", "zoom_in", "pinch_out", "zoom_out", "pinch_in", "double_tap", "long_press"], description: "预置手势名" },
      fx: { type: "number", description: "可选锚点分数 x（0~1），用于 double_tap / long_press" },
      fy: { type: "number", description: "可选锚点分数 y（0~1），用于 double_tap / long_press" },
      durationMs: { type: "number", description: "可选时长覆盖（长按/滑动，毫秒）" }
    },
    output: touchOutput,
    async execute(args, exec) {
      const preset = String(args.preset || "");
      const builder = PRESET_GESTURES[preset];
      if (builder === undefined) {
        return { ok: false, error: "未知预置手势: " + preset + "；可用：" + Object.keys(PRESET_GESTURES).join(", ") };
      }
      const fx = typeof args.fx === "number" ? Math.min(1, Math.max(0, args.fx)) : 0.5;
      const fy = typeof args.fy === "number" ? Math.min(1, Math.max(0, args.fy)) : 0.5;
      const fallback = preset === "long_press" ? 800 : (preset === "double_tap" ? 60 : 300);
      const dur = typeof args.durationMs === "number" && args.durationMs > 0 ? args.durationMs : fallback;
      const strokes = builder(fx, fy, dur);
      let total = 0;
      for (const s of strokes) {
        if (s.kind === "wait") total += typeof s.ms === "number" ? s.ms : 0;
        else if (s.kind === "tap") total += s.durationMs || 60;
        else if (s.kind === "swipe") total += s.durationMs || 300;
        else if (s.kind === "hold") total += s.durationMs || 500;
        else if (s.kind === "move" || s.kind === "up") total += s.durationMs || 100;
      }
      const raw = await a11yPost("/gesture", strokes, total + 15000);
      const v = parseResult(raw);
      if (!v.ok) return { ok: false, error: v.error || GUIDE_TEXT };
      return {
        ok: true,
