...
// 定制版（Oppo Find X7 / root）：持久化 root shell + root 专属工具
import { rootExec, rootStatus } from "./root-shell.js";
import { registerRootTools } from "./root-tools.js";
...
function privCmd(command, timeoutMs) {
  if (process.env.ROOT_AVAILABLE === "1") {
    // 定制版：root 通道改走持久化 root shell（root-shell.js）——
    // 只启动一次 su，命令间保留 cd/环境变量，且免去每次 su -c 的鉴权开销。
    return rootExec(command, timeoutMs);
  }
...
function apply(ctx) {
  ...
  if (available) {
    ctx.tools.register(defineTool({
      name: "shizuku_shell",
      description:
        "以最高系统特权执行一条 shell 命令（有 root 走**持久化 root shell**，无 root 走 Shizuku 一次性通道）。" +
        "root 通道是持久化的：多次调用之间 cd、环境变量、shell 函数都会保留，和电脑端终端一致；因此可以把复杂操作拆成多条命令逐步执行、逐步看结果，不必挤在一条里。" +
        "用于普通 bash 工具做不到的操作：读写 /data/data/<包名>/ 等应用私有目录、pm install/uninstall、am 启动/停止应用、settings/device_config 改系统设置、dumpsys 查询、grant/revoke 权限、修改 /system、iptables、重启服务等。" +
        "命令默认自动执行（无需逐次确认）；只有设了 SHIZUKU_APPROVE=ask 才逐次审批。常规文件读写与普通 shell 优先用 fs/bash 工具。" +
        "提示：会持续输出不停（如 logcat -f、ping）的命令要配 timeout 使用；命令超时后 root shell 会自动重启（cd 等状态重置）。",
      parameters: {
        command: {
          type: "string",
          required: true,
  ...
  // 1.5) 定制版：root 专属工具（仅在 root 可用时注册）
  if (process.env.ROOT_AVAILABLE === "1") {
    try {
      registerRootTools(ctx);
    } catch (e) {
      console.error("[dsh-tool-shizuku] registerRootTools 失败：", e);
    }
  }
  ...
  ctx.tools.register(defineTool({
    ...
    async execute() {
      if (process.env.ROOT_AVAILABLE === "1") {
        let st = null;
        try { st = await rootStatus(); } catch (_) {}
        return {
          available: true,
          channel: "root(su)",
          detail: st && st.available
            ? "root 通道可用，su=" + st.su + (st.persistent ? "（持久 root shell 已就绪）" : "（持久 shell 待首次调用时启动）")
            : "已授予 root 权限"
        };
      }
