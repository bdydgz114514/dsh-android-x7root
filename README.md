...
`docs/踩坑与经验.md` 收录了本次内核升级到 0.1.6-alpha.2 期间在真机上踩到的 10 个坑
（原生插件无 android 构建、浏览器会话认证、工作区弹回的真因、flock 不支持、启动提速、
Android 11+ 选文件夹被拒、CDP 调试手法等），含报错原文与实测有效的解法。
同样内容也写进了 App 的本地知识库（AI 排障时会先检索）。
