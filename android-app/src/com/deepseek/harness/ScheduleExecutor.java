...
public final class ScheduleExecutor {
    ...
    private static boolean startEngine(Context ctx) {
        try {
            ...
            // v1.7.5-x7root.4：与主引擎共用同一个外部知识库目录
            env.put("DSH_KNOWLEDGE_DIR", new File(new File(android.os.Environment.getExternalStorageDirectory(), "DeepSeekHarness"), "knowledge").getAbsolutePath());
