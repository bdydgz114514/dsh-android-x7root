...
function internalModules() {
	/*
	* Android compatibility: node-addon-require-builtin has no android-arm64 build
	* (optional packages cover darwin/linux-glibc/win32 only), so the native path
	* cannot load there and host preparation aborts before the profile tree mounts.
	* Node's own --expose-internals flag (which the DSH Android shell always passes)
	* makes the same five internal modules reachable through a plain require, so try
	* that first and keep the addon as the fallback for hosts without the flag.
	*/
	const requireInternal = (id) => {
		const req = createRequire(import.meta.url);
		if (process.execArgv.includes("--expose-internals")) try {
			return req(id);
		} catch {}
		try {
			return req("node-addon-require-builtin").requireBuiltin(id);
		} catch {}
		return req(id);
	};
	const esmModule = requireInternal("internal/modules/esm/loader");
	const cjsModule = requireInternal("internal/modules/cjs/loader");
	const cjsHelpers = requireInternal("internal/modules/helpers");
	const esmUtils = requireInternal("internal/modules/esm/utils");
	const esmResolve = requireInternal("internal/modules/esm/resolve");
