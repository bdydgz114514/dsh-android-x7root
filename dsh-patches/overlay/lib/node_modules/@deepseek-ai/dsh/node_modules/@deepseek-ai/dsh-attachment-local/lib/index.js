...
/** Android compatibility: pure JS header probe replacing sharp/libvips (no native image codec on Android). */
...
*/
function canPassThroughNormalization() {
	return true;
}
...
async function normalizeImage(data, detected) {
	/* Android compatibility: no image codec, so normalization stays a byte-identical pass-through. */
	return {
		data,
...
async function prepareImageFile(input, limits, policy) {
	...
	const normalized = await normalizeImage(input.data, detected);
...
async function createRequestImage(attachment, target, hasAlpha) {
	/* Android compatibility: no image codec, so the request version reuses the original bytes. */
	void target;
	void hasAlpha;
	return {
